...
# @Last modified time: 2020-07-11T18:17:44+10:00
...
def train(epoch, model, optimizer, train_loader, each_iteration=False, train_on_gpu=True):
    ...
    if each_iteration:
        ...
        return global_VAE_iter, kl_loss_iter, recon_loss_iter
...
def test(epoch, model, optimizer, test_loader, each_iteration=False, train_on_gpu=True):
...
def train_VAE_model(epochs, model, optimizer, train_loader, valid_loader, saving_path='best_model.pth', each_iteration=False, train_on_gpu=True):
    '''
    Params :
    ...
    if each_iteration: #True if big dataset, for which only few epochs are computed. Show results over iterations (batchs) in stead of epochs

        early_stopping = EarlyStopping(patience=2,verbose=True,path=saving_path)
        lr_schedul_VAE = torch.optim.lr_scheduler.StepLR(optimizer=optimizer, step_size=1, gamma=0.5)
        ...
        for epoch in range(model.epochs+1,model.epochs+epochs+1):
            ...
            early_stopping(np.mean(global_VAE_loss_val),VAE=model,MLP=None)
