...
path_to_CP = os.path.join(root, '200405 F1 SULF1_ECAD n1/DAPI_Phall_Sulf1_Ecad/per_cell_cropped_images')#'200405 F1 SULF1_ZEB1 n1/DAPI_Phall_Sulf1_Zeb1/per_cell_cropped_images/')

GT_csv = os.path.join(root, '200405 F1 SULF1_ECAD n1/DAPI_Phall_Sulf1_Ecad/Quantitative_Data/200405_DAPI_Phall_SULF1_ECAD_Quantitative_DataCell_Bodies.csv')# '200405 F1 SULF1_ZEB1 n1/DAPI_Phall_Sulf1_Zeb1/Quantitative_Data/200404_DAPI_Phall_SULF1_ZEB1_Quantitative_DataCell_Bodies.csv')
...
save_folder = os.path.join(path_to_save, 'Felix_Full_C4')

list_folder = ['Class_'+str(i) for i in range(2,3)] #Will consider ONLY Cluster 1 to 6
...
for index, row in CP_df.iterrows():
    ...
    img_c1_path = os.path.join(path_to_CP, f"200405_Coverslip{int(row.Metadata_Coverslip)}_XY{int(row.ImageNumber):02d}_Channel1/Cell_Bodies_{int(row.ObjectNumber)}.png")
    ...
    save_to = os.path.join(save_folder, f'Class_{class_num}', file_name_save)
    io.imsave(save_to, img_c4, plugin='tifffile')
