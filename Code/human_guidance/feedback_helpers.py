...
def latent_to_index(latents):
    metadata = {'description': 'Disentanglement test Sprites dataset.Procedurally generated 2D shapes, from 6 disentangled latent factors.This dataset uses 6 latents, controlling the color, shape, scale, rotation and position of a sprite. All possible variations of the latents are present. Ordering along dimension 1 is fixed and can be mapped back to the exact latent values that generated that image.We made sure that the pixel outputs are different. No noise added.', 'latents_sizes': np.array([ 1,
            3,  6, 40, 32, 32]), 'latents_names': ('color', 'shape', 'scale', 'orientation', 'posX', 'posY'), 'date': 'April 2017', 'version': 1, 'title': 'dSprites dataset', 'latents_possible_values': {'posX': np.array([0.,  0.03225806,  0.06451613,  0.09677419,  0.12903226,
...
class DSpritesDataset(Dataset):
  ...
  def __init__(self,imgs_subsample,GT_factors_subsample,Unique_id_subsample,feedback_csv='None',transform=[transforms.ToTensor()]):
...
def save_latent_code(model,dataloader,save_path='noname_metadata.csv',train_on_gpu=True):
    ...
    df = pd.DataFrame(columns=['Unique_id','shape','scale','orientation','posX','posY','VAE_x_coord','VAE_y_coord'])
